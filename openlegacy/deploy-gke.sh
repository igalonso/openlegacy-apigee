...
ENDPOINT=$(kubectl get service ilb-service -o json | jq '.status.loadBalancer.ingress[0].ip' -r)

cd ..
cd apigee/sharedflows/openlegacy-auth/policies
cp SC-OpenLegacy-oAuth.xml.template SC-OpenLegacy-oAuth.xml
#For docker

sed -i "s/OPENLEGACYHOST/$ENDPOINT/g" SC-OpenLegacy-oAuth.xml
